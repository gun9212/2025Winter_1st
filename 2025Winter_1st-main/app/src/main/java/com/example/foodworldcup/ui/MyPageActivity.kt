...
class MyPageActivity : BaseActivity() {
    ...
    private fun setupRecyclerView() {
        ...
        // 최신순으로 정렬 (날짜 기준 내림차순)
        val selectedFoods =
                preferenceManager.getMapSelectedFoods().sortedByDescending { it.selectedDate }
        ...
        adapter = PlateAdapter(plateList) { selectedFoodId -> showFoodDetailDialog(selectedFoodId) }
    ...
    private fun showFoodDetailDialog(selectedFoodId: Long) {
        ...
        bottomSheetBinding.dateTextView.text =
                DateFormatter.dateFormatLong.format(selectedFood.getDate())
        bottomSheetBinding.placeNameTextView.text = selectedFood.placeName.ifEmpty { "정보 없음" }
        bottomSheetBinding.placeAddressTextView.text =
                selectedFood.placeAddress.ifEmpty { "주소 정보 없음" }
        ...
        bottomSheetBinding.placeNameTextView.setOnClickListener { openPlaceDetail(selectedFood) }
