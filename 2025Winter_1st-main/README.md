...
**모든 데이터는 기기 내부에 저장되어 서버 없이 빠르고 안정적으로 동작합니다.** 게임을 진행하며 귀여운 음식 캐릭터들을 수집하고, 우승 기록과 함께 마이페이지에서 모은 캐릭터들을 확인할 수 있습니다.
...
- 🎁 **음식 캐릭터 수집**: 게임을 진행하며 귀여운 음식 캐릭터들을 모으는 재미! (결과 화면, 지도 화면, 마이페이지에서 확인)
- ⚡ **빠른 로컬 처리**: 서버 없이 기기 내부에서 모든 데이터를 처리하여 빠르고 안정적으로 동작
...
- 최종 우승 음식 표시
- 우승 음식 정보 및 이미지
- 귀여운 음식 캐릭터 표시: 우승 음식의 캐릭터 이미지 확인
- 탈락한 음식들 그리드 표시
- 다시하기 및 마이페이지 이동 버튼
- 지도에서 찾기 버튼
...
- 음식점 상세 정보 확인
- 현재 위치 기반 검색
- 음식 캐릭터 표시: 선택한 음식의 귀여운 캐릭터 이미지 확인

### 7. 마이페이지 (MyPageScreen)
- 과거 우승 기록 목록 (날짜순 정렬)
- 날짜별 우승 음식 확인
- 수집한 음식 캐릭터 모음: 과거 우승 기록과 함께 모은 귀여운 캐릭터들을 한눈에 확인
- 우승 기록 삭제 기능
- 지도에서 찾기 기능
...
### API
- **Kakao Local API**: 음식점 검색 (지도 기능에서만 사용)
- **Kakao MAP API**: 카카오 맵 불러오기 (지도 기능에서만 사용)

> **참고**: 게임의 핵심 기능(음식 선택, 토너먼트, 캐릭터 수집 등)은 모두 오프라인에서 동작하며, 인터넷 연결은 지도에서 음식점 검색 시에만 필요합니다.
...
├── assets/
│   ├── final_foods.json              # 음식 데이터 JSON (로컬 저장)
│   ├── food_images/                  # 음식 이미지 (로컬 저장)
│   ├── food_character_images/        # 음식 캐릭터 이미지 (로컬 저장, 수집 요소)
│   └── [기타 이미지 파일들]
└── res/
    └── [리소스 파일들]
```
...
2. 우승 음식 확인 및 귀여운 캐릭터 확인
3. 탈락한 음식들 그리드 확인
4. "지도에서 찾기" 버튼으로 주변 음식점 검색
5. "마이페이지"에서 우승 기록 및 수집한 캐릭터 모음 확인
6. "다시하기" 버튼으로 새 게임 시작
...
3. 현재 위치 기반 검색
4. 검색 결과 리스트 확인
5. 선택한 음식의 캐릭터 이미지 확인

### 5. 마이페이지
1. 과거 우승 기록을 날짜순으로 확인
2. 각 기록을 클릭하여 상세 정보 확인
3. 모은 음식 캐릭터들을 한눈에 확인 - 게임을 진행하며 수집한 귀여운 캐릭터 컬렉션
4. 삭제 버튼으로 기록 삭제
5. "지도에서 찾기" 버튼으로 해당 음식의 주변 음식점 검색
...
### 로컬 처리 및 성능
- **서버 없는 아키텍처**: 모든 데이터가 기기 내부에 저장되어 서버 없이 동작
- **빠른 응답 속도**: 네트워크 지연 없이 즉시 게임 진행 가능
- **오프라인 지원**: 인터넷 연결 없이도 게임 플레이 가능 (지도 기능 제외)
- **로컬 데이터 저장**: SharedPreferences와 Assets를 활용한 효율적인 데이터 관리
